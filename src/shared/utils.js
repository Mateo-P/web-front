...
export const validateForm = (formFields, formValues, error) => {
    let validator = true;
...
export const totalBill = (orders) => {
    let bill = 0;
    orders.forEach(({ item, quantity, state }) => {
        if (state !== 'DECLINED') bill += item.price * quantity;
    });
